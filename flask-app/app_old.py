...
def	getPredsForNow(preds):
	# merge predictions make copy of data
...
def mergePredictions(preds, station_status):
	# merge stations & predictions, fillna
	station_status = station_status.merge(preds, on = 'station_id',  how='left')
...
class Stations(Resource):	
	...
	def get(self):
		...
		station_status = getStationsAndStatus()

		# add predictions
		relevant_preds = getPredsForNow(preds)
		station_status = mergePredictions(preds, station_status)
...
class GeoJSON(Resource):
	@cache.cached(timeout=50)
	def get(self):
		# merge clusers with stations
		stations = getStationsAndStatus()

		# add predictions
		relevant_preds = getPredsForNow(preds)
		station = mergePredictions(preds, stations)
